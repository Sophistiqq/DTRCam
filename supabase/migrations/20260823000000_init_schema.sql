...
-- DTRCam initial database schema + RLS policies + storage buckets
...
create policy "profiles_admin_write" on public.profiles for all using (exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'));

create policy "punches_self_read" on public.punches for select using (auth.uid() = employee_id);
create policy "punches_admin_read" on public.punches for select using (exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'));

create policy "summary_self_read" on public.daily_summary for select using (auth.uid() = employee_id);
create policy "summary_admin_read" on public.daily_summary for select using (exists (select 1 from public.profiles p where p.id = auth.uid() and p.role = 'admin'));

-- 10. Storage Bucket for Punch Photos
insert into storage.buckets (id, name, public)
values ('punch-media', 'punch-media', false)
